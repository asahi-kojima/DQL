...
class Environment:
    ...
    def run(self):
        ...
        life = [ 1 for _ in range(NUM_PROCESSES)]
            ...
            for step in range(NUM_ADVANCED_STEP):
                ...
                for i in range(NUM_PROCESSES):
                    ...
                    #描画設定箇所
                        ...
                        if i == 0:
                            print(f'episode : {elapsed_episode[i]:04} | current step : {int(each_step[i]):04} | life : {life[i]+1} | rest blocks : {len(envs[i].lst):02} |',\
                              f' all envs average #reflection : {int(sum([10*full_total_ref[i]/(elapsed_episode[i]+1) for i in range(NUM_PROCESSES)])/NUM_PROCESSES)/10}')
